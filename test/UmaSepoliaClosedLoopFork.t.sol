...
contract UmaSepoliaClosedLoopForkTest is Test {
    ...
    address constant UMA_OOV2_SEPOLIA = 0x9f1263B8f0355673619168b5B8c0248f1d03e88C;
    ...
    function _contracts()
        internal
        view
        returns (UnifiedMarket market, MinimalConditionalTokens ct, MockERC20 usdc, IOptimisticOracleV2 oov2)
    {
        market = _market();
    ...
    function _prepare(MinimalConditionalTokens ct, bytes32 q) internal returns (bytes32 conditionId) {
        ct.prepareCondition(UMA_OOV2_SEPOLIA, q, 2);
        conditionId = ct.getConditionId(UMA_OOV2_SEPOLIA, q, 2);
    }

    function _splitYes(MinimalConditionalTokens ct, MockERC20 usdc, bytes32 conditionId, address user, uint256 amount)
        internal
    {
        vm.prank(user);
        ct.splitPosition2(usdc, bytes32(0), conditionId, MinimalConditionalTokens.SplitKind.YES_ONLY, amount);
    }

    function _splitNo(MinimalConditionalTokens ct, MockERC20 usdc, bytes32 conditionId, address user, uint256 amount)
        internal
    {
        vm.prank(user);
        ct.splitPosition2(usdc, bytes32(0), conditionId, MinimalConditionalTokens.SplitKind.NO_ONLY, amount);
    }
    ...
    function _umaResolvePrice(IOptimisticOracleV2 oov2, address currency, address requester, address proposer, int256 price)
        internal
        returns (int256)
    {
    ...
    vm.prank(requester);
    uint256 totalBond = oov2.requestPrice(IDENTIFIER, ts, ANCILLARY, IERC20(currency), 0);
    ...
    function _bridgeToCt(MinimalConditionalTokens ct, bytes32 q, bool yesWins) internal {
    ...
    function test_uma_case001_A_yes_B_no_yesWins_closedLoop() external {
        (UnifiedMarket market, MinimalConditionalTokens ct, MockERC20 usdc, IOptimisticOracleV2 oov2) = _contracts();
    ...
    int256 resolved = _umaResolvePrice(oov2, WETH_SEPOLIA, MM, MM, 1);
        ...
        assertEq(afterA, beforeA - 200 * USDC_DECIMALS + expectedA, "case002: A redeem mismatch");
        assertEq(afterB, beforeB - 300 * USDC_DECIMALS, "case002: B should lose");
        assertEq(afterC, beforeC - 250 * USDC_DECIMALS + expectedC, "case002: C redeem mismatch");
    ...
    uint256 beforeC = usdc.balanceOf(C);
        ...
        // MM 提供完整头寸（YES+NO）
        vm.prank(MM);
        ct.splitPosition2(usdc, bytes32(0), conditionId, MinimalConditionalTokens.SplitKind.FULL, 1000 * USDC_DECIMALS);
    ...
    _assertFinalBalance(usdc, A, beforeA, 200 * USDC_DECIMALS, expectedA, "case003: A redeem mismatch");
    assertEq(usdc.balanceOf(B), beforeB - 300 * USDC_DECIMALS, "case003: B should lose");
    _assertFinalBalance(usdc, C, beforeC, 250 * USDC_DECIMALS, expectedC, "case003: C redeem mismatch");
    _assertFinalBalance(usdc, MM, beforeMM, 1000 * USDC_DECIMALS, expectedMM, "case003: MM redeem mismatch");
